...
module.exports = createCoreController("api::article.article", ({ strapi }) => ({
  ...
  async update(ctx) {
    ...
    // article 데이터 업데이트
    const entity = await strapi.services.update({ id }, ctx.request.body);
    // 응답 반환
    return sanitizeEntity(entity, { model: strapi.models.article });
  },
  async update(ctx) {
    ...
    // user 정보는 변경할 수 없도록 처리
    if (ctx.request.body.user) return ctx.throw(400, 'user field cannot be changed');

    // 사용자의 id와 article의 작성자 id가 일치하는지 확인
    if (ctx.state.user.id !== article.user.id) return ctx.unauthorized(`You can't update this entry`);
